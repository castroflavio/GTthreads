...
gtthread_t threadidarray[max];
gtthread threadList[max];
...
void gtthread_init( long period ){
	//Set period to timequantum of schedule, initialize thread control blocks (TCB)
	init_thread_structs();
	make_sched_context();
...
int gtthread_create( gtthread_t *thread, void *(*start_routine)(void *), void *arg ){
	...
	atomic_increment( &index );
	atomic_increment( &qtyThreads );
...
void gtthread_yield( void ){
	...
	swapcontext( &threadList[currentThread].context, &scheduler);
	return;
}
